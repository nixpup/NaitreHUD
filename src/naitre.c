...
enum { CurNormal, CurPressed, CurMove, CurResize, CurInfiniteMove }; /* cursor */
...
static void apply_infinite_translation(Monitor *m, double dx, double dy,
									   uint32_t time);
...
static bool is_infinite_layout(Monitor *m);
...
static bool suppress_setfloating_arrange = false;
static Monitor *infinite_drag_mon = NULL;
static bool is_keyboard_infinite_move_active = false;
static double initial_cursor_x = 0;
static double initial_cursor_y = 0;
...
buttonpress(struct wl_listener *listener, void *data) {
	...
	case WL_POINTER_BUTTON_STATE_RELEASED:
		if (!locked && cursor_mode == CurInfiniteMove) {
			cursor_mode = CurNormal;
			infinite_drag_mon = NULL;
...
void apply_infinite_translation(Monitor *m, double dx, double dy,
								uint32_t time) {
	Client *c = NULL;
	int32_t delta_x = (int32_t)round(dx);
	int32_t delta_y = (int32_t)round(dy);

	if (!m || (!delta_x && !delta_y) || !is_infinite_layout(m))
		return;

	if (time && last_apply_drap_time != 0 &&
...
void motionnotify(uint32_t time, struct wlr_input_device *device, double dx,
	...
	if (cursor_mode == CurInfiniteMove) {
		if (is_keyboard_infinite_move_active) {
			apply_infinite_translation(
				infinite_drag_mon ? infinite_drag_mon : selmon, -dx, -dy, time);
		} else {
			apply_infinite_translation(
				infinite_drag_mon ? infinite_drag_mon : selmon, dx, dy, time);
		}
...
setfloating(Client *c, int32_t floating) {
	...
	bool should_notify = !suppress_setfloating_arrange;
...
void setup(void) {
	...
	setenv("XDG_CURRENT_DESKTOP", "naitre", 1);
	parse_config();
	init_baked_points();
