...
FuncType parse_func_name(char *func_name, Arg *arg, char *arg_value,
	...
	if (strcmp(func_name, "focusstack") == 0) {
		...
		func = toggle_monitor;
		(*arg).v = strdup(arg_value);
	} else if (strcmp(func_name, "scroller_stack_left") == 0) {
		func = scroller_stack_left;
	} else if (strcmp(func_name, "scroller_stack_right") == 0) {
		func = scroller_stack_right;
...
void parse_option(Config *config, char *key, char *value) {
	if (strcmp(key, "keymode") == 0) {
		...
		if (strcmp(value, "true") == 0 || strcmp(value, "1") == 0) {
			config->stacker_loop = 1;
		} else {
			config->stacker_loop = 0;
		}
...
void set_value_default() {
	...
	config.stacker_loop = stacker_loop;
