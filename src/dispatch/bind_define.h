...
int32_t scroller_stack_left(const Arg *arg) {
	Client *c = selmon->sel;
	if (!c || c->isfloating || !is_scroller_layout(selmon))
		return 0;

	if (c->prev_in_stack || c->next_in_stack) {
		// Window is in a stack, unstack it and move it to the left
		Client *stack_head = get_scroll_stack_head(c);
		exit_scroller_stack(c);
		wl_list_remove(&c->link);
		wl_list_insert(stack_head->link.prev, &c->link);
	} else {
		// Window is not in a stack, stack it with the window to the left
		Client *target_client = find_client_by_direction(c, &(Arg){.i = LEFT}, false, true);

		if (!target_client || target_client->mon != c->mon) {
			if (config.stacker_loop && selmon->visible_tiling_clients > 1) {
				// Find the rightmost client
				Client *iter;
				wl_list_for_each_reverse(iter, &clients, link) {
					if (VISIBLEON(iter, selmon) && ISTILED(iter)) {
						if (iter != c) {
							target_client = iter;
						}
						break;
...
int32_t scroller_stack_right(const Arg *arg) {
	Client *c = selmon->sel;
	if (!c || c->isfloating || !is_scroller_layout(selmon))
		return 0;

	if (c->prev_in_stack || c->next_in_stack) {
		// Window is in a stack, unstack it and move it to the right
		Client *stack_tail = get_scroll_stack_head(c);
		while (stack_tail->next_in_stack) {
			stack_tail = stack_tail->next_in_stack;
		}

		Client *insertion_anchor = stack_tail;
		if (c == stack_tail) {
			insertion_anchor = c->prev_in_stack;
		}
		...
		if (insertion_anchor) {
			wl_list_insert(&insertion_anchor->link, &c->link);
		} else {
			// This should not happen in a stack of 2 or more clients.
			// If it does, it means c was the only one, so just put it at the end.
			wl_list_insert(clients.prev, &c->link);
		}
	} else {
		// Window is not in a stack, stack it with the window to the right
		Client *target_client = find_client_by_direction(c, &(Arg){.i = RIGHT}, false, true);

		if (!target_client || target_client->mon != c->mon) {
			if (config.stacker_loop && selmon->visible_tiling_clients > 1) {
				// Find the leftmost client
				Client *iter;
